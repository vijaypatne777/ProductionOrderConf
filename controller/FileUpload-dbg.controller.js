				...
				psModel.read('/YY1_Prod_Ord_Comp_Stock', {
					...
					success: function (oResponse) {
						...
						for (var j = 0; j < oResponse.results.length; j++) {
							if (Quan > oResponse.results[j].MfgOrderPlannedTotalQty) {
								qError = "X";
								break;
							}
						...
						if (qError === " ") {
							for (var k = 0; k < cObj.length; k++) {
								...
								if (StockQuantity < CQuantity) {
									...
									msgObj["Mtype"] = "Error";
									msgObj["Ordno"] = Ordno;
									msgObj["Msg"] = "Material Component" + " " +  + " " + "has less quantity in" + " " + Sloc + "Storage Location.";
									that._msgArr.push(msgObj);
									break;
								}
							}
							...
							msgObj["Mtype"] = "Error";
							msgObj["Ordno"] = Ordno;
							msgObj["Msg"] = "Confirmation quantity" + " " + Quan + " " +  "is more than" + " " + oResponse.results[j].MfgOrderPlannedTotalQty + "production order quanity";
							that._msgArr.push(msgObj);
						}
						resolve();
