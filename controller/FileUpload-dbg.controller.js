...
], function (Controller, MessageToast, MessagePopover, MessageItem, Df, Button, FO, Filter, exportLibrary, Spreadsheet) {
	"use strict";
	var oMessagePopover;
	return Controller.extend("Prod_Ord_Conf.Production_Ord_Conf.controller.FileUpload", {

		onInit: function () {
            MessageToast.show("Application is loaded!!!");
			...
			this._lModel = new sap.ui.model.json.JSONModel();
					...
					var oPromise = new Promise(function (resolve, reject) {
						...
						pordModel.create('/ProdnOrdConf2', obj, {
							success: function (oResponse) {
								...
								lObj["ordno"] = obj.OrderID;
								lObj["opno"] = obj.OrderOperation;
								lObj["quan"] = obj.ConfirmationYieldQuantity;
								lObj["ctext"] = obj.ConfirmationText;
								lObj["esdate"] = obj.ConfirmedExecutionStartDate;
								lObj["estime"] = obj.ConfirmedExecutionStartTime;
								lObj["edate"] = obj.ConfirmedProcessingEndDate;
								lObj["eetime"] = obj.ConfirmedProcessingEndTime;
								lObj["pdate"] = obj.PostingDate;
								lObj["btime"] = obj.ConfirmedBreakDuration;
								lObj["pno"] = obj.Personnel;

								that._lData.push(lObj);
								resolve();
		...
		checkProdOrd: function () {
			...
			for (var i = 0; i < fData.prec.length; i++) {
				var lPromise = this.checkProdOrdStock(fData.prec[i].OrdNo, fData.prec[i].OpNo, fData.prec[i].Quan, fData.prec[i]);
				this._cPromises.push(lPromise);
			}
			return this._cPromises;

		},
		checkProdOrdStock: function (Ordno, Opno, Quan, Fdata) {
			var that = this;
				...
				psModel.read('/YY1_Prod_Ord_Comp_Stock', {
					...
					success: function (oResponse) {
						...
						for (var j = 0; j < oResponse.results.length; j++) {
							var cQuan = parseFloat(Quan);
							var oQuan = parseFloat(oResponse.results[j].MfgOrderPlannedTotalQty);
							if (cQuan > oQuan) {
								qError = "X";
								break;
							}
						...
						if (qError === " ") {
							for (var k = 0; k < cObj.length; k++) {
								...
								if (StockQuantity < CQuantity) {
									...
									lObj["msg"] = msgObj["Msg"];
									that._msgArr.push(msgObj);
									lObj["ordno"] = Fdata.OrdNo;
									lObj["opno"] = Fdata.OpNo;
									lObj["quan"] = Fdata.Quan;
									lObj["ctext"] = Fdata.CTEXT;
									lObj["esdate"] = Fdata.ValidF;
									lObj["estime"] = Fdata.ValidFT;
									lObj["edate"] = Fdata.ValidFE;
									lObj["eetime"] = Fdata.ValidFET;
									lObj["pdate"] = Fdata.PDATE;
									lObj["btime"] = Fdata.BT;
									lObj["pno"] = Fdata.PNO;
							...
							msgObj["Mtype"] = "Error";
							msgObj["Ordno"] = Ordno;
							msgObj["Msg"] = "Confirmation quantity" + " " + Quan + " " + "is more than" + " " + oResponse.results[j].MfgOrderPlannedTotalQty +
								"production order quanity";
							lObj["msg"] = msgObj["Msg"];
							that._msgArr.push(msgObj);
		...
		downloadData: function () {
			this._logData.msgs = this._lData;
			this._lModel.setData(this._logData);
			var ta = [];
			var oSet;
			var tData = {
				"msgs": ta
			};
			var tData = this._lModel.getData();
			var nData;
			if (typeof tData === "undefined") {
				nData = true;
			} else {
				if (tData.msgs.length === 0) {
					nData = true;
				}
			}
			...
			if (nData) {
				...
				aProducts = this._lModel.getProperty("/msgs");
		...
		createColumns: function () {
			var oColumns = [];
			var pordno = this.getView().getModel("i18n").getResourceBundle().getText("PordNo");
			var opno = this.getView().getModel("i18n").getResourceBundle().getText("OpNo");
			var quan = this.getView().getModel("i18n").getResourceBundle().getText("Quan");
			var ctext = this.getView().getModel("i18n").getResourceBundle().getText("Ctext");
			var esdate = this.getView().getModel("i18n").getResourceBundle().getText("ESdate");
			var estime = this.getView().getModel("i18n").getResourceBundle().getText("EStime");
			var eedate = this.getView().getModel("i18n").getResourceBundle().getText("EEdate");
			var eetime = this.getView().getModel("i18n").getResourceBundle().getText("EEtime");
			var pdate = this.getView().getModel("i18n").getResourceBundle().getText("Pdate");
			var btime = this.getView().getModel("i18n").getResourceBundle().getText("Btime");
			var pno = this.getView().getModel("i18n").getResourceBundle().getText("Pno");
